...
class LexicalAnalyser(object):
            ...
            if c == "/":  # begin of comment or /= ...
                ...
                while colIndex < len(line) and (digit.match(c)):
                    ...
                    if colIndex < len(line):
                        c = line[colIndex]
                unitValue = Integer(
                    lineIndex, beginColIndex, colIndex - beginColIndex, n
                )
            elif space.match(c):
                colIndex = colIndex + 1
            elif char.match(c):
    ...
    def acceptKeyword(self, keyword):
        if not self.verify_index():
            raise AnaLexException(
                "Found end of entry while keyword " + keyword + " expected!"
            )
        if self.lexical_units[self.lexical_unit_index].is_keyword(keyword):
            ...
            raise AnaLexException(
                ...
                + keyword
                + " <line "
                + str(self.lexical_units[self.lexical_unit_index].get_line_index())
                + ", column "
                + str(self.lexical_units[self.lexical_unit_index].get_col_index())
                + "> !"
            )
    ...
    def acceptCharacter(self, c):
        if not self.verify_index():
            raise AnaLexException(
                "Found end of entry while expecting character " + c + "!"
            )
    ...
    def acceptSymbol(self, s):
        if not self.verify_index():
            raise AnaLexException(
                "Found end of entry while expecting symbol " + s + "!"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Do the lexical analysis of a NNP program."
    )
    parser.add_argument(
